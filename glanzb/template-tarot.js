...
var Card = (function(){
	...
	function Card(id) {  //Card is also IIFE's internal name
        ...
        this.id = id; //personal property, saving it in the nstance
    }		// Card is a constructor, needs "this"
    ...
    Card.prototype.isValid = function() { // Returns--> true, false
        ...
        //         && (this.id%1 === 0)        //integer
        //         && 
        return this.id>=0 && this.id<=51;   //in range
    };		// this.id without id as a parameter
	...
	Card.isCard = function(thing) {// return Boolean
        ...
        return (!thing.isValid) &&
       			(!thing.isValid()) &&
       			(thing.isValid === Card.prototype.isValid) &&
       			(thing instanceof Card);
       
    };//...

	Card.fullSet = function() {//return copy of private array
		return fullSet.slice(1); 
	}
...
var TarotCard = (function(){ //<-- Superclass is parameter, but equals Card
	...
	rankNames.unshift(" "); // add ana empty value to the beginning;
	//unshift makes it longer, add a value to the begining - shift takes one off
