...
var Card = (function(){
	...
	Card.prototype.isValid = function(){
        ...
        if(typeof(input) != 'number' || input%1 != 0){
            return false
        } else if(input < 0 || input > this.constructor.numCards() - 1){
            return false
        } else {
            return true
        }
    ...
    Card.prototype.name = function() { //--> string, NaN
        var rankName = this.constructor.rankNames()[this.rank() -1];
        var suitName = this.constructor.suitNames()[this.suit() -1];
	...
	Card.isCard = function(thing) {// return Boolean
		if(!(thing instanceof Card)){return false}
		if(!thing.isValid()){return false};
		return true;
	}
